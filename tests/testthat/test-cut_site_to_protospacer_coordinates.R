test_that("cut_site_to_protospacer_coordinates works for + strand", {
  result <- cut_site_to_protospacer_coordinates(
    cut_site = 100,
    strand = '+',
    protospacer_distance_from_cut_PAM_distal = 20,
  ...
  expect_equal(result$protospacer_end, 81)
})
...
test_that("cut_site_to_protospacer_coordinates works for - strand", {
  ...
  expect_equal(result$protospacer_end, 119)
})

test_that("cut_site is coerced to numeric", {
  result <- cut_site_to_protospacer_coordinates(cut_site = "100",strand = "+")
  expect_equal(result$protospacer_start, 103)
})
