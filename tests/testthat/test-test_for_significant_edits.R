...
testthat::test_that("test_for_significant_edits with Ns and non on-target edit", {
  dfx <- data.frame(
    position = c(1:5, 5, 6),
    base = c('G', 'A', 'T', 'C', 'N', 'G', 'A'),
    reference_base = c('A', 'A', 'T', 'C', 'A', 'A', 'T'),
    position_depth = c(20, 20, 20, 20, 20, 20, 20),
    base_count = c(10, 2, 1, 1, 10, 10, 20),
    p_value = c(0.01, 0.2, 0.5, 0.001, 0.001, 0.001, 0.001),
    conf_low = c(0.2, 0.01, 0.0, 0.0, 0, 0, 0),
    conf_high = c(0.8, 0.3, 0.1, 0.1, 1, 1, 1),
    percentage = c(50, 10, 5, 5, 50, 50, 100),
    stringsAsFactors = FALSE
  )

  # Positive strand test
  result_pos <- test_for_significant_edits(dfx)
  testthat::expect_true(all(c("all", "significant") %in% names(result_pos)))
